document.addEventListener('DOMContentLoaded', () => {
      ...
      const res = await fetch(`http://10.218.172.40:3000/api/usuarios/admin/alterar-senha/${idUsuarioAlterandoSenha}`, {
        method: 'POST',
        headers: {
      ...
      if (isAdmin) {
        fetch('http://10.218.172.40:3000/api/usuarios/admin/users', {
      ...
      async function updateUserPermission(userId, field, value) {
        try {
          const token = localStorage.getItem('token');
          const response = await fetch(`http://10.218.172.40:3000/admin/update-permission`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
      ...
      async function excluirUsuario(id) {
        ...
        try {
          const token = localStorage.getItem('token');
          await fetch(`http://10.218.172.40:3000/api/usuarios/admin/excluir/${id}`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${token}` }
          });
