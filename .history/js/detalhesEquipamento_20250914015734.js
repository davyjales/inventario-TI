...
function displayEquipmentDetails(equipment, additionalFields) {
  ...
  // Campos adicionais
  if (additionalFields && additionalFields.length > 0 && equipment.additionalFields) {
    const labelMap = {};
...
function printQRCodeForZebra() {
  if (!globalEquipment) {
    alert('Dados do equipamento não carregados.');
    return;
  }
...
function setupEditButton(equipmentId) {
  ...
  // Mantém o modal de edição da versão atual
