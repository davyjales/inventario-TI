      ...
      equipamentosBody.querySelectorAll('.btn-edit').forEach(button => {
        button.addEventListener('click', async () => {
          ...
          try {
            ...
            async function renderAdditionalFields(additionalFields, categoriaId) {
              ...
              if (additionalFields && Object.keys(additionalFields).length > 0) {
                try {
                  ...
                  // Create a map from nome_campo to display name
                  const nomeCampoMap = {};
                  camposMeta.forEach(campo => {
                    // Use campo.nome_campo as key and campo.nome_campo as display name (or adjust if display name differs)
                    nomeCampoMap[campo.nome_campo] = campo.nome_campo;
                  });
                  ...
                  Object.entries(additionalFields).forEach(([key, value]) => {
                    ...
                    // Transform key like "campo_2" to "Campo 2" for better display
                    ...
                    label.textContent = nomeCampoMap[key] ? formatFieldName(nomeCampoMap[key]) : formatFieldName(key);
            ...
            renderAdditionalFields(equipamento.additionalFields);
