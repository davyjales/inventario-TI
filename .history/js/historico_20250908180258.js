document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    console.log("🔎 changed_fields bruto:", item.changed_fields);

    const changesObj = JSON.parse(item.changed_fields);

    console.log("✅ changed_fields parseado:", changesObj);
    ...
    changesText = Object.entries(changesObj)
      ...
      .map(([key, value]) => {
        if (key === 'additionalFields') {
          console.log("📦 additionalFields recebido:", value);

          return Object.entries(value)
            .map(([campoId, change]) => {
              console.log("👉 Campo detectado:", campoId, change);

              // Normaliza ID (remove "campo_" se existir)
              const cleanId = Number(campoId.replace('campo_', ''));

              const nomeCampo =
                additionalFieldsMap.get(cleanId) || `Campo ${campoId}`;

              console.log("📝 Nome resolvido:", nomeCampo);
