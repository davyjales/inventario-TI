document.addEventListener('DOMContentLoaded', () => {
  ...
  // Função para limpar o código vindo do QR ou do input
  function sanitizeCode(code) {
    let equipmentId = code.trim();

    // Se tiver underline, pega só a parte antes dele
    if (equipmentId.includes("_")) {
      equipmentId = equipmentId.split("_")[0];
    }

    // Remove .png caso exista no final
    equipmentId = equipmentId.replace(".png", "");
  ...
  async function processScannedCode(code) {
    try {
      const equipmentId = parseInt(sanitizeCode(code));
