  ...
  async function loadMappings() {
    try {
      const [catRes, statusRes, cargoRes, camposRes] = await Promise.all([
        ...
        fetch('/api/status'),
        fetch('/api/cargos'),
        fetch('/api/campos-adicionais') // nova rota para pegar os campos adicionais
      ]);
      if (!catRes.ok || !statusRes.ok || !cargoRes.ok || !camposRes.ok) throw new Error('Erro ao carregar mapeamentos');
      const [categories, statuses, cargos, campos] = await Promise.all([catRes.json(), statusRes.json(), cargoRes.json(), camposRes.json()]);
