...
if (key === 'additionalFields') {
  // Show only the changes present in changed_fields for additionalFields
  const additionalChanges = value;
return Object.entries(additionalChanges)
  .map(([campoId, change]) => {
    const nomeCampo = additionalFieldsMap.get(Number(campoId)) || `Campo ${campoId}`;
    return `<div>👉 ${nomeCampo}: de <em>${change.de || 'N/A'}</em> para <em>${change.para || 'N/A'}</em></div>`;
  })
  .join('');
}
             else {
