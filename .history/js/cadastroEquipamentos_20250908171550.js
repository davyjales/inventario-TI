...
document.addEventListener("DOMContentLoaded", () => {
      ...
      if (campos.length > 0) {
        campos.forEach(async (campo) => {
          ...
          switch (campo.tipo) {
            ...
            if (campo.opcoes && Array.isArray(campo.opcoes) && campo.opcoes.length > 0) {
              campo.opcoes.forEach(opcao => {
                const option = document.createElement("option");
                option.value = opcao.value;
                option.textContent = opcao.label;
                input.appendChild(option);   // ✅ agora está certo
              });
            } else {
              const opt = document.createElement("option");
              opt.value = "";
  ...
  categoriaSelect.addEventListener("change", async (e) => {
    ...
    if (categoria && categoria.recebe_termo) {
      ...
      console.log("Mostrando upload de termo.");
    } else {
      uploadTermoContainer.style.display = "none";
      console.log("Ocultando upload de termo.");
    }
  });
