...
function displayEquipmentDetails(equipment, additionalFields) {
    ...
    if (additionalFields && additionalFields.length > 0) {
        // Create a mapping from generic keys to proper field names
        const keyToNameMap = {};
        additionalFields.forEach(field => {
            keyToNameMap[field.nome_campo] = field.nome_campo;
        });

        html += '<div class="detail-section"><h3>Campos Adicionais</h3><div class="field-group">';
        for (const key in equipment.additionalFields) {
            const label = keyToNameMap[key] || key;
            html += `
                <div class="field-item">
