...
async function exportEquipmentTableToMultipleSheets(table, filename) {
  ...
  try {
    // Usar equipamentos filtrados se disponíveis, senão buscar todos
    let allEquipments;
    if (window.filteredEquipmentsData && window.filteredEquipmentsData.length > 0) {
      allEquipments = window.filteredEquipmentsData;
    } else {
      ...
      allEquipments = await equipmentResponse.json();
    }
    ...
    // Exportar equipamentos filtrados ou todos
