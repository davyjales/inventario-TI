document.addEventListener('DOMContentLoaded', () => {
  ...
  pageData.forEach(item => {
          ...
          changesText = realChanges.map(([key, value]) => {
            if (key === 'additionalFields') {
              ...
              return [...allCampoIds].map(campoId => {
                ...
                const newVal = newValues[campoId] || 'N/A';
              ...
              const oldVal = value.de || 'N/A';
              const newVal = value.para || 'N/A';
            ...
            <div style="margin-bottom: 20px;">
              ...
              <table style="width: 100%; border-collapse: collapse;">
                <tr style="background-color: #f8f9fa;">
                  ...
                  <td style="padding: 8px; border: 1px solid #ddd;">${item.equipment_name || 'N/A'}</td>
                </tr>
                <tr>
                  <td style="padding: 8px; font-weight: bold; border: 1px solid #ddd;">Usuário:</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${item.dono || 'N/A'}</td>
                </tr>
                <tr style="background-color: #f8f9fa;">
                  <td style="padding: 8px; font-weight: bold; border: 1px solid #ddd;">Setor:</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${item.full_snapshot?.setor || 'N/A'}</td>
                </tr>
                <tr>
                  <td style="padding: 8px; font-weight: bold; border: 1px solid #ddd;">Categoria:</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${item.full_snapshot?.categoria_nome || 'N/A'}</td>
                </tr>
                <tr style="background-color: #f8f9fa;">
                  <td style="padding: 8px; font-weight: bold; border: 1px solid #ddd;">Status:</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${item.full_snapshot?.status_nome || 'N/A'}</td>
                </tr>
                <tr>
                  <td style="padding: 8px; font-weight: bold; border: 1px solid #ddd;">Cargo:</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${item.full_snapshot?.cargo || 'N/A'}</td>
                </tr>
                <tr style="background-color: #f8f9fa;">
                  <td style="padding: 8px; font-weight: bold; border: 1px solid #ddd;">Descrição:</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${item.full_snapshot?.descricao || 'N/A'}</td>
                </tr>
              </table>
            </div>
    ...
    try {
        ...
        if (typeof obj === 'object' && obj !== null) {
          ...
          if (parentKey === 'additionalFields') {
            return Object.entries(obj).map(([campoId, campoData]) => {
              const nomeCampo = additionalFieldsMap.get(Number(campoId)) || `Campo ${campoId}`;
              const valor = (typeof campoData === 'object' && campoData.valor !== undefined)
                ? campoData.valor
                : campoData;
          ...
          return '<ul>' + Object.entries(obj).map(([k, v]) => {
            ...
            // Mapeamento de nomes dos campos
            if (k === 'categoria_nome') displayKey = 'Categoria';
              ...
              displayKey = 'QRCode';
              displayValue = String(v).replace('.png', ''); // Remove .png do QRCode
            }
      ...
      // Se não há snapshot, mostrar mensagem
      if (!snapshot || Object.keys(snapshot).length === 0) {
        modalBody.innerHTML = '<em>Snapshot não disponível para este registro.</em>';
      }
    } catch (error) {
