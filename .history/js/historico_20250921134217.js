        ...
        if (typeof obj === 'object' && obj !== null) {
          ...
          if (parentKey === 'additionalFields') {
            return Object.entries(obj).map(([campoId, campoData]) => {
              // Tentar extrair um ID numérico do campoId:
              ...
              // Nome do campo: priorizar mapping das categorias, senão usar campoData.nome, senão fallback para a chave bruta
              const nomeCampo = (numericId !== null && !isNaN(numericId) ? additionalFieldsMap.get(Number(numericId)) : null)
                                || (campoData && campoData.nome)
                                || campoId;

              // Se campoData é objeto com 'nome' e 'valor', renderizar nome: valor diretamente
              if (typeof campoData === 'object' && campoData !== null && 'nome' in campoData && 'valor' in campoData) {
                return `<li><strong>${nomeCampo}</strong>: ${campoData.valor || 'Não informado'}</li>`;
              }
