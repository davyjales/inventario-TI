  ...
  status_nome: 'Status Nome' // Add this to handle the duplicate
      ...
      if (item.action === 'update') {
        try {
          ...
          // Filter out the status_nome entry
          delete changesObj.status_nome; // Remove the duplicate entry
          // Filter out unnecessary fields
          const relevantKeys = ['status_id', 'cargo']; // Add relevant keys here
          changesText = Object.entries(changesObj)
            .filter(([key]) => relevantKeys.includes(key)) // Only include relevant keys
            .map(([key, value]) => {
