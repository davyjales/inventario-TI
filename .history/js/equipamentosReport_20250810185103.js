document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyFilters() {
    filteredData = equipamentosData.filter(eq => {
      return Array.from(columnFilters).every(input => {
        ...
        // Garante que a propriedade existe no objeto
  ...
  // Adicione este evento para cada input de filtro
  columnFilters.forEach(input => {
    input.addEventListener('input', () => {
      // Limpa o timeout anterior se existir
      if (input.timeout) clearTimeout(input.timeout);
      
      // Adiciona um pequeno delay para não filtrar a cada tecla pressionada
