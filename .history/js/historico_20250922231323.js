  ...
  pageData.forEach(item => {
    ...
    if (item.action === 'update') {
      try {
        ...
        console.log('🔍 Debug - changed_fields recebidos:', changesObj);
        ...
        console.log('🔍 Debug - mudanças filtradas:', realChanges);
          ...
          changesText = realChanges.map(([key, value]) => {
            if (key === 'additionalFields') {
              ...
              return [...allCampoIds].map(campoId => {
                ...
                // Só mostrar se o nome do campo não for um campo técnico (campo_14, etc)
                if (nomeCampo && !nomeCampo.startsWith('campo_') && nomeCampo !== campoId && nomeCampo !== `Campo ${campoId}`) {
              ...
              let label = key;
              let oldVal = 'Não informado';
              let newVal = 'Não informado';

              if (typeof value === 'object' && value.campo) {
                label = value.campo;
                oldVal = value.de || 'Não informado';
                newVal = value.para || 'Não informado';
              } else if (typeof value === 'object' && (value.de !== undefined || value.para !== undefined)) {
                oldVal = value.de || 'Não informado';
                newVal = value.para || 'Não informado';
              } else {
                // Se não for objeto com de/para, considera como valor único
                newVal = String(value);
              }
        ...
        console.error("❌ Dados recebidos:", item.changed_fields);
    ...
    try {
      ...
      // Para campos adicionais, renderizar individualmente
          if (parentKey === 'additionalFields') {
            return Object.entries(obj).map(([campoKey, campoValue]) => {
              // Determinar o nome do campo
              let nomeCampo = null;

              // Tentar mapear por ID numérico
              const numericId = Number(campoKey);
              if (!isNaN(numericId)) {
                nomeCampo = additionalFieldsMap.get(numericId);
              }

              // Se não encontrou, usar a chave como nome (pode já ser o nome de exibição)
              if (!nomeCampo) {
                nomeCampo = campoKey;
              }

              // Determinar o valor
              let valor = campoValue;
              if (typeof campoValue === 'object' && campoValue !== null && 'valor' in campoValue) {
                valor = campoValue.valor;
              }

              // Renderizar se tiver nome e valor válido
              if (nomeCampo && valor !== null && valor !== undefined && valor !== '') {
                return `<li><strong>${nomeCampo}</strong>: ${valor}</li>`;
              }
