      ...
      equipamentosBody.querySelectorAll('.btn-edit').forEach(button => {
        button.addEventListener('click', async () => {
          ...
          try {
            ...
            function renderAdditionalFields(additionalFields) {
              ...
              if (additionalFields && Object.keys(additionalFields).length > 0) {
                Object.entries(additionalFields).forEach(([key, value]) => {
                  ...
                  // Fix label to show proper field name without "campo_" prefix
                  // Also handle keys like "campo_2" or "2" by mapping to proper field names if possible
                  // Here, just display the key as is, assuming backend sends correct keys
