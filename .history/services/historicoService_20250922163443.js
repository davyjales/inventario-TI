      ...
      for (let i = 0; i < historico.length; i++) {
        ...
        try {
          ...
          if (record.action === 'consulta') {
            ...
            // Adicionar campos adicionais como objeto chave-valor
            snapshot.additionalFields = {};
            equipamentoAntes
              .filter(eq => eq.campo_id)
              .forEach(eq => {
                const nomeCampo = eq.nome_exibicao || `Campo ${eq.campo_id}`;
                snapshot.additionalFields[nomeCampo] = eq.valor || 'N/A';
              });
          ...
          equipamentoAtual.forEach(eq => {
            if (eq.campo_id) {
              const nomeCampo = eq.nome_exibicao || `Campo ${eq.campo_id}`;
              fallbackSnapshot.additionalFields[nomeCampo] = eq.valor || 'N/A';
            }
          });
