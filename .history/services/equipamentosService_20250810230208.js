...
module.exports = {
    ...
    try {
      ...
      const [result] = await connection.query(
        `INSERT INTO equipamentos 
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [categoria_id, nome, dono, setor, descricao, termo, identificador, status_id || 1, cargo || null]
      );
  ...
  async listarEquipamentos(req, res) {
    ...
    let query = `
      SELECT e.id, e.categoria_id, e.nome, e.dono, e.setor, e.descricao, e.termo, e.hostname, e.status_id, e.cargo,
      ...
      await connection.query(
        `UPDATE equipamentos
         SET categoria_id = ?, nome = ?, dono = ?, setor = ?, descricao = ?,
             termo = ?, status_id = ?, status = (SELECT nome FROM status_equipamentos WHERE id = ?), cargo = ?
         WHERE id = ?`,
        [
