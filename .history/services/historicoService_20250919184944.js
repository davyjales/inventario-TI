      ...
      for (let i = 0; i < historico.length; i++) {
        ...
        try {
          ...
          if (record.action === 'consulta') {
            ...
            // Para update, construir o snapshot acumulativo
            const prevSnapshot = i > 0 ? snapshots[historico[i - 1].id] : {};

            // Para cada campo no changed_fields, usar o valor 'para' (novo valor)
            const currentSnapshot = { ...prevSnapshot };
            for (const [key, value] of Object.entries(changedFields)) {
              if (value && typeof value === 'object' && value.para !== undefined) {
                currentSnapshot[key] = value.para;
              } else {
