        ...
        try {
          ...
          for (const [field, change] of Object.entries(changedFields)) {
            if (field === 'categoria_id') {
              ...
              if (categoriaId) {
                // Buscar o nome do campo adicional correto pelo nome do campo interno (field)
                const [categoriaCampoRows] = await db.query(
                  'SELECT nome_exibicao FROM categoria_campos_adicionais WHERE categoria_id = ?',
                  [categoriaId]
                );
                // Mapear nome_exibicao para nome_exibicao (ajuste para usar nome_exibicao como chave)
                const nomeCampoMap = {};
                categoriaCampoRows.forEach(campo => {
                  nomeCampoMap[campo.nome_exibicao] = campo.nome_exibicao;
                });
  
                // Para cada campo alterado, usar o nome_exibicao correto
