        ...
        try {
          ...
          if (categoriaId) {
            const [additionalRows] = await db.query(
              `SELECT nome_exibicao, nome FROM categoria_campos_adicionais WHERE categoria_id = ?`,
              [categoriaId]
            );
          ...
          additionalFields.forEach(f => {
            fieldNameToDisplayName[f.nome] = f.nome_exibicao;
          });

          for (const [field, change] of Object.entries(changedFields)) {
            if (field === 'categoria_id') {
              // Buscar nome da categoria pelo id
              const [categoriaRows] = await db.query('SELECT nome FROM categorias WHERE id = ?', [change.de]);
              const [categoriaRowsPara] = await db.query('SELECT nome FROM categorias WHERE id = ?', [change.para]);
              enrichedChanges['Categoria'] = {
                de: categoriaRows.length > 0 ? categoriaRows[0].nome : change.de,
                para: categoriaRowsPara.length > 0 ? categoriaRowsPara[0].nome : change.para
              };
