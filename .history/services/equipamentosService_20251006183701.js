      ...
      if (additionalFields) {
        ...
        if (campos.length > 0) {
          ...
          for (const campo of campos) {
            ...
            const [uniqueRows] = await connection.query('SELECT conteudo_unico FROM categoria_campos_adicionais WHERE id = ?', [campoId]);
            if (uniqueRows.length > 0 && uniqueRows[0].conteudo_unico) {
              const [existing] = await connection.query(
                `SELECT e.id FROM equipamentos e
                 ...
                 WHERE e.categoria_id = ? AND eca.campo_id = ? AND eca.valor = ?`,
                [categoria_id, campoId, valor]
              );
              if (existing.length > 0) {
                throw new Error(`Valor '${valor}' já existe para o campo único '${uniqueRows[0].nome_exibicao || campoId}'.`);
              }
            }
          }
