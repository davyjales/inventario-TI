...
module.exports = {
  ...
  async criarEquipamento(req, res) {
    const { categoria_id, nome, dono, setor, descricao, status_id, additionalFields } = req.body;
    ...
    try {
      ...
      const [result] = await connection.query(
        `INSERT INTO equipamentos 
         (categoria_id, nome, dono, setor, descricao, termo, qrCode, status_id) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [categoria_id, nome, dono, setor, descricao, termo, identificador, status_id || 1]
      );
  ...
  async buscarEquipamentoPorId(req, res) {
    ...
    try {
      const [results] = await db.query(
        `SELECT id, categoria_id, nome, dono, setor, descricao, termo, qrCode, hostname, status_id
         FROM equipamentos 
         WHERE id = ?`,
        [id]
      );
  ...
  async atualizarEquipamento(req, res) {
    ...
    try {
      ...
      const camposPermitidos = ['categoria_id', 'nome', 'dono', 'setor', 'descricao', 'status_id'];
