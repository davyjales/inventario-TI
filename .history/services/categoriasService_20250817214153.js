...
module.exports = {
  ...
  async getListaOptions() {
    const query = 'SELECT value, label FROM lista_opcoes'; // Adjust the query based on your database schema
  ...
  async criarCategoria(req, res) {
    ...
    try {
      ...
      if (Array.isArray(campos) && campos.length > 0) {
        const listaOptions = campos.filter(c => c.tipo === 'lista');
        ...
        // Insert options for each lista type field
        for (const campo of listaOptions) {
          if (campo.opcoes && campo.opcoes.length > 0) {
            const optionsValues = campo.opcoes.map(opcao => [campo.id, opcao.value, opcao.label]);
            await connection.query(
