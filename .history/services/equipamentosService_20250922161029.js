 ...
 async listarEquipamentos(req, res) {
  ...
  async atualizarEquipamento(req, res) {
    ...
    try {
          ...
          if (Array.isArray(camposAdicionais)) {
            ...
            if (camposAdicionais.length > 0) {
              for (const campo of camposAdicionais) {
                await connection.query(
                  'INSERT INTO equipamento_campos_adicionais (equipamento_id, campo_, valor) VALUES (?, ?, ?)',
            );
            }
          }
        } catch (e) {
      ...
      // --- LOG: usar logEquipmentChange (somente depois de aplicar todas as alterações) ---
