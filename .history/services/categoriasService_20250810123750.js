      ...
      const categoriasComCampos = await Promise.all(
        categorias.map(async cat => {
          const [campos] = await db.query(
            'SELECT id, nome_campo, obrigatorio FROM categoria_campos_adicionais WHERE categoria_id = ?',
            [cat.id]
          );
          return { ...cat, campos };
        })
    ...
    try {
      ...
      if (Array.isArray(campos) && campos.length > 0) {
        ...
        await connection.query(
          'INSERT INTO categoria_campos_adicionais (categoria_id, nome_campo, obrigatorio) VALUES ?',
          [values]
        );
      }
