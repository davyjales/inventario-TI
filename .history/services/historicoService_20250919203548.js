  ...
  async listarHistorico(req, res) {
    try {
      ...
      const [equipamentosAtuais] = await db.query(
        `SELECT e.id, e.nome, e.dono, e.setor, e.descricao, e.cargo, e.qrcode, e.status_id, e.categoria_id,
      ...
      for (let i = 0; i < historico.length; i++) {
        ...
        try {
          ...
          if (record.action === 'consulta') {
            ...
            // Para update, construir o snapshot do momento anterior à alteração
            ...
            // Se não houver 'de', usar o valor atual (significa que o campo não mudou)
            for (const [key, value] of Object.entries(changedFields)) {
              if (value && typeof value === 'object' && value.de !== undefined) {
                currentSnapshot[key] = value.de;
              }
              // Se não há valor 'de', mantém o valor atual (campo não foi alterado)
            }
