        ...
        try {
          ...
          if (categoriaId) {
            const [additionalRows] = await db.query(
              `SELECT nome_exibicao FROM categoria_campos_adicionais WHERE categoria_id = ?`,
              [categoriaId]
            );
          ...
          additionalFields.forEach(f => {
            fieldNameToDisplayName[f.nome_exibicao] = f.nome_exibicao;
          });

          for (const [field, change] of Object.entries(changedFields)) {
            // Para todos os campos, buscar nome exibido e valores reais
            ...
            if (categoriaId) {
              ...
              // Mapear nome_exibicao para nome_exibicao
              const nomeCampoMap = {};
              categoriaCampoRows.forEach(campo => {
                nomeCampoMap[campo.nome_exibicao] = campo.nome_exibicao;
              });
  
              nomeExibicao = nomeCampoMap[field] || field;
            }
          ...
          // Adicionar campo Status uma única vez, se existir
          ...
          for (const [field, change] of Object.entries(changedFields)) {
            if (field === 'status_id' || field === 'cargo') {
              // Já tratado acima ou manter valor atual
              if (field === 'cargo') {
            ...
            // Para todos os outros campos, buscar nome exibido e valores reais
            let nomeExibicao = field;
            if (categoriaId) {
              // Buscar todos os campos adicionais da categoria
              const [categoriaCampoRows] = await db.query(
