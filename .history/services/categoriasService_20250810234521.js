...
module.exports = {
  ...
  async obterCategoriaPorId(req, res) {
    ...
    try {
      ...
      const [campos] = await db.query(
        'SELECT id, nome_campo, tipo, obrigatorio, conteudo_unico FROM categoria_campos_adicionais WHERE categoria_id = ? ORDER BY id ASC',
        [id]
      );
  ...
  async criarCategoria(req, res) {
    ...
    try {
      ...
      if (Array.isArray(campos) && campos.length > 0) {
        const values = campos.map(c => [categoriaId, c.nome_campo, c.tipo, !!c.obrigatorio, !!c.conteudo_unico]);
        await connection.query(
          'INSERT INTO categoria_campos_adicionais (categoria_id, nome_campo, tipo, obrigatorio, conteudo_unico) VALUES ?',
          [values]
        );
      }
  ...
  async atualizarCategoria(req, res) {
    ...
    try {
      ...
      if (Array.isArray(campos)) {
        ...
        if (campos.length > 0) {
          const values = campos.map(c => [id, c.nome_campo, c.tipo, !!c.obrigatorio, !!c.conteudo_unico]);
          await connection.query(
            'INSERT INTO categoria_campos_adicionais (categoria_id, nome_campo, tipo, obrigatorio, conteudo_unico) VALUES ?',
            [values]
          );
  ...
  async listarCamposPorCategoria(req, res) {
    ...
    try {
      const [campos] = await db.query(
        'SELECT id, nome_exibicao, nome_campo, tipo, obrigatorio, conteudo_unico FROM categoria_campos_adicionais WHERE categoria_id = ? ORDER BY id ASC',
        [id]
      );
      res.json(campos);
    } catch (err) {
