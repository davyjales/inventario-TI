      ...
      for (let i = 0; i < historico.length; i++) {
        ...
        try {
          ...
          if (record.action === 'consulta' || record.action === 'create') {
            ...
            // Para update, se changed_fields já contiver mudanças formatadas (com campos de/para),
            // usar diretamente; caso contrário, buscar estado atual
            ...
            if (hasFormattedChanges) {
              // Se já tem mudanças formatadas, usar como snapshot
              snapshots[record.id] = changedFields;
            } else if (Object.keys(changedFields).length === 0) {
              // Se changed_fields estiver vazio, buscar estado atual
              ...
              // Se changed_fields não estiver vazio mas não tem formato de mudanças, usar como snapshot
              snapshots[record.id] = changedFields;
            }
          }
        } catch (e) {
