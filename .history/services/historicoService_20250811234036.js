      ...
      for (const record of historico) {
        try {
          ...
          const [additionalFields] = await db.query(
            ...
            [record.equipment_id ? (await db.query('SELECT categoria_id FROM equipamentos WHERE id = ?', [record.equipment_id]))[0][0].categoria_id : null]
          );
          ...
          for (const [field, change] of Object.entries(changedFields)) {
            if (field === 'categoria_id' || field === 'status_id' || field === 'cargo') {
              ...
              // Get display name from categoria_campos_adicionais by internal field name
              const [campoRows] = await db.query(
                `SELECT nome_exibicao FROM categoria_campos_adicionais WHERE nome_campo = ? LIMIT 1`,
                [field]
              );
