          ...
          for (const [field, change] of Object.entries(changedFields)) {
            if (field === 'categoria_id') {
              ...
              const [categoriaCampoRows] = await db.query(
                'SELECT nome_exibicao FROM categoria_campos_adicionais WHERE nome = ? AND categoria_id = ?',
                [field, categoriaId]
              );
              const nomeExibicao = categoriaCampoRows.length > 0 ? categoriaCampoRows[0].nome_exibicao : field;
