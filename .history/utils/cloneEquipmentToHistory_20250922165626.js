...
async function cloneEquipmentToHistory(connection, equipmentId, action, userId) {
  ...
  // Debug: log dos campos encontrados na query
  console.log('Campos equipamento query result:', camposEquipamento);

  // Se não encontrou campos com campo_id, tentar buscar todos os campos da tabela
  if (camposEquipamento.length === 0) {
    console.log('Nenhum campo adicional encontrado com campo_id, verificando estrutura da tabela...');
    const [allCampos] = await connection.query(
      'SELECT * FROM equipamento_campos_adicionais WHERE equipamento_id = ?',
      [equipmentId]
    );
    console.log('Todos os campos da tabela:', allCampos);
  }
  ...
  // Debug: log dos campos adicionais encontrados
  console.log('Campos adicionais encontrados para equipamento', equipmentId, ':', additionalFieldsObj);
  console.log('Mapa de campos da categoria:', campoIdToDisplayName);
