pshot // utils/cloneEquipmentToHistory.js
...
async function cloneEquipmentToHistory(connection, equipmentId, action, userId, changes = null) {
  ...
  // Para todas as ações, salvar o snapshot completo no changed_fields
  // Para update, incluir também as mudanças específicas se fornecidas
  let dataToSave = { ...snapshot };

  if (action === 'update' && changes) {
    // Para update, adicionar as mudanças específicas ao snapshot
  ...
  await connection.query(
    `INSERT INTO equipment_history (equipment_id, action, changed_fields, user_id)
     VALUES (?, ?, ?, ?)`,
    [equipmentId, action, JSON.stringify(dataToSave), userId]
  );
}
