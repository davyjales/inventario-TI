...
async function cloneEquipmentToHistory(connection, equipmentId, action, userId, changes = null) {
  ...
  if (action === 'create' || action === 'consulta') {
    ...
    // Para update, salvar as mudanças calculadas no changed_fields
    // Se mudanças foram fornecidas, usá-las; caso contrário, usar objeto vazio
    const changesToSave = changes || {};
    await connection.query(
      `INSERT INTO equipment_history (equipment_id, action, changed_fields, user_id)
       VALUES (?, ?, ?, ?)`,
      [equipmentId, action, JSON.stringify(changesToSave), userId]
    );
  }
}
