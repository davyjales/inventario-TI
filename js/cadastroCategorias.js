...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Função para lidar com a mudança do tipo de campo
  window.handleFieldTypeChange = function(selectElement) {
    const campoItem = selectElement.closest('.campo-item');
    const tipoSelecionado = selectElement.value;

    // Verifica se o tipo selecionado é "lista"
    if (tipoSelecionado === "lista") {
      // Adiciona um campo para opções da lista
      let optionsContainer = campoItem.querySelector('.options-container');
      if (!optionsContainer) {
        optionsContainer = document.createElement('div');
        optionsContainer.classList.add('options-container');
        optionsContainer.innerHTML = `
          <input type="text" placeholder="Opções (separadas por vírgula)" class="campo-opcoes" />
        `;
        campoItem.appendChild(optionsContainer);
      }
    } else {
      // Remove o campo de opções se não for "lista"
      const optionsContainer = campoItem.querySelector('.options-container');
      if (optionsContainer) {
  ...
  function criarCampoInput(campo = {}) {
    ...
    div.innerHTML = `
      ...
      <select class="campo-tipo" onchange="handleFieldTypeChange(this)">
    ...
    if (campo.tipo === "lista" && campo.opcoes && campo.opcoes.length > 0) {
      let optionsContainer = div.querySelector('.options-container');
      if (!optionsContainer) {
        optionsContainer = document.createElement("div");
        optionsContainer.classList.add("options-container");
        optionsContainer.innerHTML = `
          <input type="text" placeholder="Opções (separadas por vírgula)" class="campo-opcoes"
                value="${campo.opcoes.map(o => o.label || o).join(", ")}" />
        `;
  ...
  categoriaForm.addEventListener("submit", async (e) => {
    ...
    camposContainer.querySelectorAll(".campo-item").forEach(div => {
      ...
      if (nome_campo) {
        ...
        if (tipo === "lista") {
          ...
          if (opcoesInput) {
            const opcoes = opcoesInput.value
              .split(",")
              .map(o => o.trim())
              .filter(o => o.length > 0);
            campoData.opcoes = opcoes; // adiciona opções no payload
          }
        }
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Erro ao salvar categoria: ${errorData.details || errorData.error || 'Erro desconhecido'}`);
      }
    } catch (err) {
      console.error("Erro ao salvar categoria:", err);
      if (err.name === 'TypeError' && err.message === 'Failed to fetch') {
        alert('Erro de conexão com o servidor. Por favor, verifique se o servidor está rodando e tente novamente.');
      } else {
