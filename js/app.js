document.addEventListener('DOMContentLoaded', () => {
    ...
    categorias.forEach(cat => {
        ...
        editBtn.addEventListener('click', () => {
            ...
            if (novoNome && novoNome.trim() !== '' && novoNome !== cat.nome) {
                // For simplicity, not editing additional fields here
                fetchWithAuth(`/categorias/${cat.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ nome: novoNome.trim(), additionalFields: cat.additionalFields })
                }).then(res => {
    ...
    document.querySelectorAll('.delete-icon').forEach(icon => {
        icon.addEventListener('click', async () => {
            ...
            try {
                const res = await fetchWithAuth(`/equipamentos/${id}`, { 
                    method: 'DELETE',
                    headers: {
    ...
    // Add export to Excel button and handler
    const reportSection = document.getElementById('consulta');
    if (reportSection) {
        // Remove existing export button if any to avoid duplicates
        const existingBtn = reportSection.querySelector('.export-excel-btn');
        if (existingBtn) {
            existingBtn.remove();
        }
        ...
        exportBtn.addEventListener('click', async () => {
            try {
                const res = await fetchWithAuth('/equipamentos/export/excel');
                if (!res.ok) throw new Error('Erro ao exportar para Excel');
                const blob = await res.blob();
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'equipamentos.xlsx';
        ...
        reportSection.querySelector('.report-header').appendChild(exportBtn);
    }
    }
    ...
    async function loadEquipmentHistory() {
        try {
            const response = await fetchWithAuth('/api/equipment/history');
    ...
    function renderHistoryTable(history) {
        ...
        history.forEach(item => {
            ...
            // Formata as alterações
            let changesHtml = '';
            if (item.action === 'update' && item.changed_fields) {
                changesHtml = '<div class="change-details"><ul>';
                for (const [field, values] of Object.entries(item.changed_fields)) {
                    changesHtml += `<li><strong>${field}:</strong> ${values[0] || 'N/A'} → ${values[1] || 'N/A'}</li>`;
                }
                changesHtml += '</ul></div>';
            ...
            // Formata a ação
            const actionText = item.action === 'create' ? 'Criação' : 
                            item.action === 'update' ? 'Atualização' : item.action;
